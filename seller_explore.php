...
// Fetch all available products randomly (not limited to just seller's products)
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .product-description {
            max-width: 100% ; /* Add this line to stop before right buttons */
            padding-right: 0rem;          /* Optional: extra visual space from right edge */
            max-height: none;
            font-size: 0.9rem;
            margin-bottom: 0.5rem;
            display: none;
...
<body>
    <header>
        ...
        <a href="seller_home.php" class="back-button"><i class="fas fa-chevron-left"></i></a>
    </header>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Action button (ellipsis) - opens product management options
            document.querySelectorAll('.action-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const productId = this.dataset.productId;
                    // Check if this is the seller's product
                    fetch(`check_product_owner.php?product_id=${productId}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.is_owner) {
                                // Open seller's product management page
                                window.location.href = `seller_edit_product.php?id=${productId}`;
                            } else {
                                // Just show reviews for other sellers' products
